 ...
 * already-built ProseMirror document instead of `HeaderFooter.content`. The
 * painter's `getHfPmDoc` seam (see `computeLayout`) routes here when a caller
 * supplies a live PM doc for an HF slot; otherwise the painter parses
 * `HeaderFooter.content` via {@link convertHeaderFooterToContent}.
 *
 * @public
 */
