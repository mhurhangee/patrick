...
**Status (OPEN=CONTEXT reshape — built across `feat/open-context-engine` → `feat/per-doc-context-mode` → `feat/remove-roam-tools`):** the engine now embodies the model. `<OPENDOCUMENTS>` pushes each open doc in full (original/derivations/notes) honouring a per-doc `ContextMode` (original/derivations/both), set from a menu on each PDF's chat chip; `<CLOSEDDOCUMENTS>` surfaces the closed-doc triage roster; `readFile`/`listDirectory` (the silent text-roam pull channel) are **removed** — the agent suggests "open X" instead. **Next:** derivations (Summarise/Analyse/Translate) build into this as the cheap representation. (Not yet: persisting `ContextMode` across sessions; a soft cap on closed-doc volume.)
