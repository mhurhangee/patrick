...
> **⚠️ BEING REMOVED (decided 2026-06-06).** The derivation concept is reverted — see the Context model "Status" note above. Extraction/summaries fed to the AI created a trust gradient that fights OPEN=CONTEXT. ExtractPat + this whole framework get ripped out (archived at `archive/derivations-extractpat`); summary/analysis/translation become AI-written **notes/artifacts** the attorney curates, not typed derivations. The section below describes the code as it still exists until the rip-out lands.
...
**Status — the model is now a strict BINARY (decided 2026-06-06; branch `feat/open-context-binary`):** a deliberate reversal of the "derivations as cheap representation" idea. Feeding a derived layer (extraction/summary) alongside the source created a *trust gradient* — the agent got two representations of one fact and didn't know which to believe (and the token-saving rationale for derivations is a cloud-SaaS concern that *inverts* for a local, bill-by-the-hour, accuracy-first tool: attorneys would rather a frontier model read the real PDF than trust a cheap summary). So:
- **Open doc = the real source, full stop** — PDF as a file part / artifact text, **plus its notes**. Nothing derived between the agent and the document. (The per-doc `ContextMode` toggle was removed — there's nothing to toggle.)
- **Closed doc = a cheap signpost only** — filename, file type, and any note (the note *is* the awareness layer). Never its content. The agent triages and suggests "open X".
- `readFile`/`listDirectory` already removed (silent-roam). `extractSource` tool still wired transitionally — goes when ExtractPat is ripped out.

**In flight (see task list / [[context-model]]):** validate this binary (re-test) → **rip out ExtractPat + the whole derivation framework** (preserved at tag/branch `archive/derivations-extractpat`) → notes become human+AI with a signpost callout node, always sent → a task-setup/source-management screen for naming/signposting/excluding sources. The Derivations section below is **being removed** — kept until the rip-out lands.
...
(`readFile`/`listDirectory` were **removed** — they were the silent text-roam pull channel that contradicted OPEN=CONTEXT; `<CLOSEDDOCUMENTS>` now provides the folder awareness instead, and the agent suggests "open X" rather than silently reading.)
...
- **Tokens (implemented):** context `<ATTORNEY>` `<PRACTICECONTEXT>` `<TASK>` `<EXCLUDED>` `<DOCTYPE>` `<LOCATIONINSTRUCTION>`; scope `<OPENDOCUMENTS>` `<CLOSEDDOCUMENTS>` `<CURRENTSOURCE>`; tools `<EXTRACTSOURCE>` `<FETCHPATENT>`. Add a token = catalog entry (shared) + resolver (api). The scope tokens **embody the binary Context model**: `<OPENDOCUMENTS>` = open docs in full (source + notes, no derivations), `<CLOSEDDOCUMENTS>` = a cheap signpost (filename, type, note) — never content. (Retired the old hint-list/auto-push tokens `<OPENSOURCES>`/`<EXISTINGEXTRACTIONS>`/`<EXTRACTEDDATA>`/`<NOTES>` and the silent-roam tools `<READFILE>`/`<LISTDIRECTORY>`; `ContextMode`/per-doc toggle and `<FOCUSEDSOURCES>`/`<SOURCETEXT>` were tried and dropped. `<EXTRACTSOURCE>` goes with the ExtractPat rip-out.)
