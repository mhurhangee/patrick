...
- **Artifacts** — documents drafted in Plate, saved to `artifacts/` as `.json` (+ `.docx`). Attorney edits in Word if they want. (Manual creation/editing works; AgentPat authoring tools are being (re)built on the mechanical-applicator model — see AgentPat § Authoring model.)
...
**Authoring model — decided 2026-06-07 (branch `feat/agentpat-authoring`), not yet built.** AgentPat *is* the author (**mechanical-applicator / "Claude Code for patents"**), not a hand-off to a second writer. The document is the model's **output, expressed as the tool's input** — content rides in the tool-call *arguments*; the tool is a dumb sink (file the bytes / render in the editor / return a tiny ack). Tracked changes need **no second brain** — `oldText → newText` → Plate suggestion marks is pure code (word-diff one block).
- `writeArtifact(content)` — creation; content rides in args, streamed into the artifact panel live (outside the chat bubble — the structured successor to v0's `<CODE>` tags).
- `editBlock(oldText → newText)` — surgical tracked-change edit; the unit is the **block, not the file**. Code word-diffs that one block → suggestion marks. **Verification:** `oldText` must match the real block or the edit is rejected + retried (grounds it in truth; this is Claude Code's `old_string` check). Block-by-block runs as **one agent conversation, many tool-call turns** (doc sent once + prompt-cached; only changed blocks re-emitted) — NOT N independent calls/subagents.
- Both are **client-side HITL tools** (no server `execute`), reusing the confirm-card + tool-loop plumbing. See the [[artifact-drafting]] memory.

The earlier **second-author** approach (AgentPat relays a `brief` → DraftPat re-writes it) was tried and **abandoned** (double-write, homogenisation, hand-off boundary, source paid twice); archived at branch `archive/writeartifact-second-author`. DraftPat/NotePat stay as the *user-driven* in-editor AI only — not AgentPat's ghostwriter.

**Planned (not yet built):** the authoring tools above; `readDocx`/`writeDocx`, USPTO `fetchOfficeAction`/`fetchGlobalDossier`.
