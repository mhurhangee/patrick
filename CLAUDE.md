...
**Current vs target:** today open PDFs are pushed as file parts for every open source (no per-doc toggle yet), closed docs aren't surfaced, and `readFile`/`listDirectory` give the agent silent text roam (predates this model → to be re-examined). Reshaping the engine tokens + the per-doc toggle + closed-triage is the next work; derivations build into it.
...
- **Tokens (implemented):** context `<ATTORNEY>` `<PRACTICECONTEXT>` `<TASK>` `<EXISTINGEXTRACTIONS>` `<EXCLUDED>` `<EXTRACTEDDATA>` `<NOTES>` `<DOCTYPE>` `<LOCATIONINSTRUCTION>`; scope `<OPENSOURCES>` `<CURRENTSOURCE>`; tools `<EXTRACTSOURCE>` `<READFILE>` `<LISTDIRECTORY>` `<FETCHPATENT>`. Add a token = catalog entry (shared) + resolver (api). **This vocabulary is hint-lists + auto-push fragments and is being reshaped to the Context model** (open-content-by-toggle + closed-triage); `<FOCUSEDSOURCES>`/`<SOURCETEXT>` were tried and reverted (branch `feat/focused-source-tokens`).
