...
- The UI label is derived per-surface (DraftPat/NotePat) but the command *list* is still shared — a future per-surface split (see `prompt-templates-plan` memory follow-ups). The `askpat-*` localStorage keys kept their names (cosmetic, deferred rename).
