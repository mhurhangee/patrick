...
- **H/F bugs found in the audit, to fix during this pass:** even-page headers aren't actually editable (only `first`/`default` hdrFtrType threaded; `even` is cast-accepted but never produced/selectable — `useHeaderFooterEditing`); `(entry as any).hfRid` untyped mutation in `DocxEditor` trackedChanges memo; four near-identical HF anchor-position loops in `useLayoutPipeline` (dedupe); `scrollToChangeId`/`scrollToParaId` HF-fallback branches set selection but don't scroll the HF view.
...
- core `ClipboardManager` + `AutoSaveManager` + `TableSelectionManager` are now orphaned: their only consumers were the react `useClipboard`/`useAutoSave`/`useTableSelection` hooks, deleted in the dead-hooks + dead-table-path culls. They remain exported from core's public barrel (`managers/index.ts` + `core.ts`). [low] *Trigger:* a core public-surface pass — decide keep-as-public-API vs remove (also covers the `managers/types.ts` Clipboard/AutoSave/TableSelection option/snapshot types). Removing them is a core-export change, out of scope for the react culls.
